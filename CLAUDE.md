...
- ✅ **verify-slip ย้ายมา worker แล้ว**: โหมด API เรียก `/api/payments/verify-slip` บน worker (fallback โหมดปิด API = Pages Function เดิม) · **รอใส่ secret 2 ตัวให้ worker**: `cd backend` แล้ว `npx wrangler secret put SUPABASE_SERVICE_ROLE_KEY` + `npx wrangler secret put EASYSLIP_API_TOKEN` (มีผลทันที ไม่ต้อง deploy ใหม่) - service_role หาได้ที่ Supabase Dashboard -> Project Settings -> API Keys -> Legacy API keys · EasySlip token ที่ developer.easyslip.com
