...
// details = ข้อมูลเสริมให้ frontend เอาไปไฮไลต์รายการที่มีปัญหาได้ (เช่น สินค้าที่สต็อกไม่พอ)
export class HttpError extends HTTPException {
  constructor(status: number, message: string, public code = 'error', public details?: unknown) {
    super(status as any, { message })
  }
}

export const badRequest = (m: string) => new HttpError(400, m, 'bad_request')
export const outOfStock = (m: string, details?: unknown) => new HttpError(409, m, 'out_of_stock', details)
...
export function errorHandler(err: Error, c: Context) {
  if (err instanceof HttpError) {
    return c.json({ ok: false, code: err.code, error: err.message, ...(err.details !== undefined && { details: err.details }) }, err.status as any)
  }
