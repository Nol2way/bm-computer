...
จุดเด่นของโครงงานคือ ทุกฟังก์ชันสามารถใช้งานได้จริง โดยการสั่งซื้อจะบันทึกข้อมูลลงฐานข้อมูลทันที การชำระเงินผ่าน QR PromptPay รองรับการตรวจสอบสลิปอัตโนมัติกับผู้ให้บริการจริง และการเพิ่มหรือแก้ไขข้อมูลสินค้าในระบบหลังบ้านจะปรากฏบนหน้าเว็บทันทีที่ลูกค้าเปิดหน้านั้น โดยไม่ต้อง build หรือแก้โค้ดใหม่
...
## ความสามารถของระบบสำหรับลูกค้า

- 🛍️ **เลือกซื้อสินค้ากว่า 150 รายการ จาก 39 แบรนด์จริง** - ซีพียู การ์ดจอ โน้ตบุ๊ก จอ เกมมิ่งเกียร์ ครบในที่เดียว
- 🔎 **ระบบค้นหาและกรองสินค้า** - รองรับการค้นหาแบบคลาดเคลื่อน (Fuzzy Search) เพื่อช่วยให้สามารถค้นหาสินค้าได้แม้สะกดคำไม่ถูกต้อง (เช่นพิมพ์ "corsiar" ก็เจอ Corsair) พร้อมตัวกรองหมวด แบรนด์ ช่วงราคา
...
- 👥 **ระบบแบ่งปันชุดสเปกคอมพิวเตอร์** - เปิดดูสเปคที่คนอื่นจัดไว้ แล้วกดนำไปใช้ต่อได้เลย
- 🧾 **ใบกำกับภาษี** - กรอกที่อยู่ใบกำกับภาษีตอนสั่งซื้อได้ทันที ยืนยันที่อยู่แล้วเปิดดู/พิมพ์ใบกำกับภาษีย้อนหลังได้
- 🛡️ **เคลมประกันสินค้า** - ยื่นเคลมพร้อมแนบรูปหลักฐานได้จากในบัญชี ระบบตรวจให้เองว่าสินค้าชิ้นไหนยังอยู่ในระยะประกัน และติดตามสถานะอนุมัติได้ในหน้าบัญชี
...
## ความสามารถของระบบสำหรับผู้ดูแลระบบ

- 📊 **ภาพรวมร้าน** - ยอดขายรายวันเป็นกราฟ ออเดอร์แยกตามสถานะ สินค้าขายดี และแจ้งเตือนสต็อกใกล้หมด
- 📦 **จัดการสินค้า/หมวด/แบรนด์/สไลด์โปรโมชัน** - เพิ่มแล้วขึ้นหน้าเว็บทันที ไม่ต้องแก้โค้ด รวมถึงกำหนดเงื่อนไข/ระยะเวลารับประกันของแต่ละสินค้าได้
- 🚚 **จัดการออเดอร์** - อัปเดตสถานะ ใส่เลขพัสดุ อนุมัติการยกเลิก/คืนเงิน (ระบบตัด-คืนสต็อกให้อัตโนมัติ)
- 🛡️ **จัดการเคลมประกัน** - ตรวจสอบคำขอเคลมพร้อมรูปหลักฐาน อนุมัติ/ปฏิเสธ พร้อมสรุปจำนวนตามสถานะ
...
ระบบถูกออกแบบโดยแบ่งออกเป็น 3 องค์ประกอบหลัก ได้แก่ ส่วนติดต่อผู้ใช้ (Frontend) ระบบประมวลผล (Backend API) และฐานข้อมูล (Database)

```mermaid
flowchart LR
    U["ลูกค้า / แอดมิน<br/>มือถือ หรือ คอมพิวเตอร์"]
    FE["หน้าร้าน (เว็บไซต์)<br/>หน้าเว็บที่ทุกคนเห็นและกดใช้งาน<br/>สร้างด้วย React"]
    API["พนักงานร้าน (ระบบกลาง)<br/>รับคำสั่ง ตรวจสิทธิ์ ตรวจสลิป<br/>ก่อนบันทึกหรือส่งข้อมูลกลับ<br/>ทำงานบน Cloudflare Worker"]
    DB[("สมุดบัญชีร้าน (ฐานข้อมูล)<br/>สินค้า ออเดอร์ สมาชิก สต็อก<br/>เก็บบน Supabase")]

    U -->|"เปิดเว็บ หรือ กดสั่งซื้อ"| FE
    FE -->|"ส่งคำขอ"| API
    API -->|"อ่าน หรือ บันทึกข้อมูล"| DB
    DB -->|"ข้อมูลล่าสุด"| API
    API -->|"ผลลัพธ์"| FE
```

**แปลว่าอะไร:** ส่วนติดต่อผู้ใช้จะไม่เชื่อมต่อฐานข้อมูลโดยตรง ทุกคำขอจะถูกส่งผ่านระบบ Backend เพื่อดำเนินการตรวจสอบสิทธิ์ ประมวลผล และบันทึกข้อมูลก่อนส่งผลลัพธ์กลับมายังผู้ใช้งาน
...
flowchart LR
    ...
    G --> H["รับสินค้าเรียบร้อย<br/>กลับมารีวิวและเคลมประกันได้"]
```
...
- 🛒 **ตะกร้าแยกตามบัญชี** - ตะกร้าผูกกับรหัสผู้ใช้ ล็อกอินคนละบัญชีบนเครื่องเดียวกันจะไม่เห็นตะกร้าของอีกคน และถ้าสลับบัญชีในอีกแท็บ ทุกแท็บจะอัปเดตตามทันที
- 📦 **ขายเกินจำนวนที่มีจริงไม่ได้** - กันไว้ 3 ชั้น: หน้าเว็บปิดปุ่มซื้อเมื่อของหมด · ระบบกลางปฏิเสธคำสั่งซื้อที่ของไม่พอ · ฐานข้อมูลมีกฎห้ามสต็อกติดลบ ต่อให้มีคนกดจ่ายพร้อมกันก็ตัดเกินไม่ได้ (การตัดสต็อกกับการเปลี่ยนสถานะเป็น "ชำระแล้ว" ทำในธุรกรรมเดียว)
- 🖼️ **รูปหลักฐานเคลมประกันเป็นความลับ** - เก็บในที่เก็บไฟล์แบบปิด เปิดดูได้ผ่านลิงก์ชั่วคราวอายุ 10 นาทีที่ออกให้เฉพาะเจ้าของเคลมกับแอดมินเท่านั้น
...
```mermaid
flowchart LR
    P["1. วางแผน<br/>กำหนดขอบเขต ฟีเจอร์<br/>กลุ่มผู้ใช้"] --> A["2. วิเคราะห์<br/>Persona และ Use Case<br/>ความต้องการระบบ"]
    A --> D["3. ออกแบบ<br/>หน้าจอ Wireframe<br/>ฐานข้อมูล และสถาปัตยกรรม"]
    D --> I["4. พัฒนา<br/>หน้าเว็บ + ระบบกลาง<br/>+ ฐานข้อมูล"]
    I --> T["5. ทดสอบ<br/>ทดลองใช้จริงทุกฟีเจอร์<br/>เก็บผล UAT"]
    T --> M["6. ส่งมอบ + ดูแล<br/>ขึ้นเว็บจริงอัตโนมัติ<br/>ทุกครั้งที่แก้โค้ด"]
...
| ส่วน | วิธี deploy |
|------|-------------|
| หน้าเว็บ (Cloudflare Pages) | push ขึ้น GitHub `main` แล้วระบบ build + ขึ้นเว็บให้อัตโนมัติ |
| **ระบบกลาง (Cloudflare Worker)** | **ต้องสั่งเองทุกครั้ง:** `cd backend && npx wrangler deploy` |

> ⚠️ **ข้อควรระวัง:** Worker **ไม่** อัปเดตตาม GitHub อัตโนมัติเหมือนหน้าเว็บ
> ถ้าเพิ่ม/แก้ endpoint แล้วลืมสั่ง deploy หน้าเว็บจริงจะเรียก API ไม่เจอ (404) ทั้งที่โค้ดถูกต้องแล้ว
> เช็คว่า worker เป็นเวอร์ชันล่าสุดได้ที่ [`/api/docs`](https://bm-computer.pages.dev/api/docs) ว่ามี endpoint ที่เพิ่งเพิ่มครบไหม
