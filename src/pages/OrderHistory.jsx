...
export default function OrderHistory() {
  ...
  return (
    <div className={`${wrap} py-6`}>
      ...
      {!user ? (
        <Empty icon="user" title={t('orders.loginToView')}><button onClick={() => openAuth('login')} className="mt-5 rounded-xl bg-brand-600 px-6 py-3 font-semibold text-white hover:bg-brand-700">{t('auth.signin')}</button></Empty>
      ) : loading ? (
        <div className="py-16 text-center text-muted">{t('common.loading')}</div>
      ) : orders.length === 0 ? (
        <Empty icon="receipt" title={t('orders.empty')}><Link to="/products" className="mt-5 rounded-xl bg-brand-600 px-6 py-3 font-semibold text-white hover:bg-brand-700">{t('orders.shopNow')}</Link></Empty>
      ) : (
