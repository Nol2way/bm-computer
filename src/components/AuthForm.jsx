...
export default function AuthForm({ view, setView, onClose }) {
  ...
  const canSubmit = isLogin ? f.email && f.password : (f.fullName && f.phone && f.email && pw.valid && !mismatch)
    ...
    try {
      if (isLogin) {
        ...
        const { error } = await supabase.auth.signUp({
          ...
          options: { data: { full_name: f.fullName, phone: f.phone } },
        })
        if (error) throw error
  ...
  return (
    <div className="rounded-2xl border border-line bg-surface p-7 shadow-2xl">
      ...
      <form onSubmit={submit} className="flex flex-col gap-4">
        {!isLogin && (
          <>
            <div><label className="mb-1.5 block text-sm font-semibold">{t('auth.fullName')}</label><input className={input} value={f.fullName} onChange={set('fullName')} placeholder="สมชาย ใจดี" autoComplete="name" /></div>
            <div><label className="mb-1.5 block text-sm font-semibold">{t('auth.phone')}</label><input className={input} value={f.phone} onChange={set('phone')} type="tel" inputMode="tel" placeholder="08x-xxx-xxxx" autoComplete="tel" /></div>
          </>
        )}
